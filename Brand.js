...
function renderTable(brands) {
  ...
  brands.forEach(brand => {
    ...
    const imageUrl = brand.image && isValidUrl(brand.image) ? brand.image : 'Not null';
    rowElement.innerHTML = `
      <td class="px-4 py-3 text-gray-800">${sanitizeInput(brand.id) || 'N/A'}</td>
      <td class="px-4 py-3">
        <img src="${imageUrl}" alt="Brand Logo" class="w-10 h-10 rounded-full" onerror="this.src='Not null'">
      </td>
